...
export default class FlightsAddorEdit extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    // console.log(JSON.stringify(nextProps.itemData));
    // console.log(that.state.itemDataString);
    if(that.state.itemDataString != JSON.stringify(nextProps.itemData)){
      ...
      that.setState({
        ...
        nameValue:nameValue,
        plantseattypes:nextProps.itemData &&nextProps.itemData.plantseattypes ? nextProps.itemData.plantseattypes : [],
  ...
  submitClick() {
    ...
    data.seattypes = that.state.plantseattypes.map((item)=>item.code);
    ...
    if(data.plantseattypes.length==0){
  ...
  setCheckData(bodyListData){
    ...
    // that.FlightsUserList=[]
    // that.props.fuserList.forEach((item,index)=>{
    //   that.state.fuserList.forEach((itemData)=>{
    //     if(item.userid == itemData.userid){
    //       that.FlightsUserList.push(item)
    //       that.griduser.select('userid',item.userid,true)
    //     }
    //   })
    // })
  }
